...
class UniswapDataCollector:
    ...
    def __init__(self, token_address: str):
        ...
        self.uniswap_fetcher.fetch_uniswap_token(token_address)
...
def token_data_collect_task(token_name: str, token_address: str) -> None:
    ...
    current_time = int(time.time())
    latest_obtained_time  = collector.fetch_subgraph_data(current_time - SEVEN_DAY_SECONDS)
...
if __name__ == "__main__":
    for name, address in TOKENS.items():
        t = threading.Thread(target=token_data_collect_task, args=(name, address,))
        t.start()
    SERVICE_BARRIER.wait()
    print("🚀 Backfill complete - starting API service and continying to poll 🚀")
